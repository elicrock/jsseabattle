    ...
    generateShip() {
      for (let i = 0; i < this.optionShip.count.length; i++) {
        for (let j = 0; j < this.optionShip.count[i]; j++) {
          ...
          const ship = this.generateOptionsShop(size);
          this.ships.push(ship);
          this.shipCount++;
        }
      }
    },
    generateOptionsShop(shipSize) {
      const ship = {
        hit: [],
        location: [],
      };
      ...
      if (direction) {
        x = Math.floor(Math.random() * COL_AREA);
        y = Math.floor(Math.random() * (ROW_AREA - shipSize));
      } else {
        x = Math.floor(Math.random() * (COL_AREA - shipSize));
        y = Math.floor(Math.random() * ROW_AREA);
      }
      ...
      if (this.checkCollision(ship.location)) {
        return this.generateOptionsShop(shipSize);
      }
      ...
      for (let i = 0; i < location.length; i++) {
        ...
        for (let j = startCoordX; j < startCoordX + 3; j++) {
          ...
          for (let z = startCoordY; z < startCoordY + 3; z++) {
            if (j >= 0 && j < COL_AREA && z >= 0 && z < ROW_AREA) {
              const coord = j + '' + z;
